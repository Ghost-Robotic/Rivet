...
//#define YmaxRestriction 250

//#define ZminRestriction 0
//#define ZmaxRestriction 200

//#define XminRestriction -100
//#define XmaxRestriction 100
...
void joystickCalibrate() {
  YpotMiddle = analogRead(XcoordPin);
  ZpotMiddle = analogRead(YcoordPin);
  XpotMiddle = analogRead(ZcoordPin);
...
void findCoords()
{
  YpotVal = analogRead(3);
  ZpotVal = analogRead(0);
  XpotVal = analogRead(5);
  EpotVal = analogRead(1);
...
void servoMove()
{
  if (joint_0_Angle<=180, joint_0_Angle>=0, joint_1_Angle<=180, joint_1_Angle>=0, joint_2_Angle<=180, joint_2_Angle>=0) {
    servo0_Ms0 = map(joint_0_Angle, 0, 180, 470, 2280);  
